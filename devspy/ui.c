...
static HDWP layout_children(HWND main_window, HDWP defer)
{
	...
	for (HWND hwnd = GetWindow(main_window, GW_CHILD); hwnd; hwnd = GetWindow(hwnd, GW_HWNDNEXT))
	{
		...
		case ID_HWND_MESSAGES_LIST:
		{
			...
			defer = DeferWindowPos(defer, hwnd, NULL, 0, 0, splitter_rect.left, client.bottom, swp_flags | (visible ? SWP_SHOWWINDOW : SWP_HIDEWINDOW));
		} break;
...
static void update_list_column_widths(HWND list_hwnd)
{
	...
	// ListView_GetHeader
	// 	then Header_GetItemCount

	for (int i = 0; i < _countof(list_column_widths); ++i)
	{
		if (list_column_widths[i] < 0)
		{
			list_column_widths[i] = from_pixels(-list_column_widths[i], dpi);
		}

		ListView_SetColumnWidth(list_hwnd, i, list_column_widths[i]);
	}
}
...
static LRESULT CALLBACK main_window_proc(HWND hwnd, uint32_t msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_PAINT:
    {
        ...
        RECT client, splitter;
        if (get_layout_rects(hwnd, &client, &splitter))
        {
            FillRect(hdc, &splitter, GetSysColorBrush(COLOR_3DFACE));
        }
...
bool init_ui(HINSTANCE instance, app_data_t* data)
{
	...
	HWND main_window = CreateWindowEx(
		0, wc_init.lpszClassName, L"", WS_OVERLAPPEDWINDOW,
